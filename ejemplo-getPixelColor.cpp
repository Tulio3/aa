...
int main()
{
	...
	// lee el color del pixel en la posicion (x,y) y lo guarda en color
	ventana.GetPixelColor(x, y, &color);
	ventana.printf(50, 100, "pixel color = RGB(%d, %d, %d)", GetRValue(color), GetGValue(color), GetBValue(color));
